...
import { BottomSheet, Button, FieldLabel, TextField } from '../ui/kit';
...
export function SubscriptionSheet() {
  ...
  const subscribe = () => {
    ...
    if (digits.length < 12 || !/^\d{2}\/\d{2}$/.test(exp) || cvv.replace(/\D/g, '').length < 3 || !holder.trim()) {
  ...
  return (
    <BottomSheet title="SplitFlik Plus" onClose={closeSubscription}>
      ...
      <TextField value={card} onChange={(e) => setCard(e.target.value)} inputMode="numeric" placeholder="4242 4242 4242 4242" autoComplete="off" style={{ marginBottom: 12 }} />
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        <div style={{ flex: 1 }}>
          <FieldLabel>Poteče</FieldLabel>
          <TextField value={exp} onChange={(e) => setExp(e.target.value)} inputMode="numeric" placeholder="MM/LL" autoComplete="off" />
        </div>
        <div style={{ flex: 1 }}>
          <FieldLabel>CVV</FieldLabel>
          <TextField type="password" value={cvv} onChange={(e) => setCvv(e.target.value)} inputMode="numeric" placeholder="123" autoComplete="off" />
        </div>
      </div>
